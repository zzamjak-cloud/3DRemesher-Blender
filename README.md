...
| 범용 QuadriFlow 경로 | 격자 미지원 형상은 복셀 리메시(촘촘히) → 파편 제거 → Decimate → 매니폴드 수리 → 자식 Blender 프로세스의 QuadriFlow(시드·시간 제한 재시도) → 작은 구멍 쿼드 메움 → 원본 표면 투영 슈링크랩과 릴랙스로 처리합니다. 대칭은 양의 반쪽만 경계를 보존해 깔고 미러 용접합니다. 결과는 원본 표면 거리 4%·비다양체 0·종횡비 20·대칭 오차 검사를 거칩니다. 밀도 속성과 `DIRECTION` 가이드는 반영하지 않으며, 복셀 리메시가 좁은 틈과 내부 면을 닫을 수 있습니다. |
