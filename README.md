...
게임 모델용 쿼드 리토폴로지를 목표로 개발 중인 Blender Extension입니다. **0.6.0은 평면 패치, 독립된 열린 관, 제한된 단일 구형 두부, 축에 정렬된 단일 T형 연결부에 새 쿼드 격자를 만듭니다. 불규칙하게 삼각화된 관의 단면 링, 돌출형 합성 얼굴의 눈·입·코·귀 가이드 폐루프 여섯 개, 엄격한 원형 단면의 합성 T형 연결부를 다룹니다. 그 밖의 일반 형상은 복셀 리메시와 QuadriFlow 로 새 쿼드 와이어를 깔고 원본 표면에 투영하는 범용 경로로 처리합니다. 이 범용 경로는 균일한 쿼드를 만들며, 팔·다리 둘레에 둔 닫힌 `LOOP` 가이드를 절단 링으로 써서 그 위치에 링 격자를 만듭니다. 그 밖의 얼굴·관절 엣지 루프 배치는 보장하지 않으며, 실제 캐릭터의 변형 품질은 검증되지 않았습니다.** 원본을 보존하고 결과를 별도 오브젝트로 만듭니다.

핵심 생성 방식을 [루프·격자 중심의 새 엔진 설계](docs/topology-redesign.md)로 재설계했습니다. 아래 표는 0.6.0의 실제 범위를 나타냅니다.
...
| 범용 QuadriFlow 경로 | 격자 미지원 형상은 복셀 리메시(촘촘히) → 파편 제거 → Decimate → 매니폴드 수리 → 자식 Blender 프로세스의 QuadriFlow(시드·시간 제한 재시도) → 작은 구멍 쿼드 메움 → 원본 표면 투영 슈링크랩과 릴랙스로 처리합니다. 대칭은 양의 반쪽만 경계를 보존해 깔고 미러 용접합니다. 결과는 원본 표면 거리 4%·비다양체 0·종횡비 20·대칭 오차 검사를 거칩니다. 닫힌 `LOOP` 가이드는 절단 링이 됩니다: 루프 평면 ±(출력 엣지 절반)의 얇은 띠를 지우고 경계를 보존해 QuadriFlow 를 돌리면 원통에서 임의 각도로 굳던 방향장이 경계에 정렬돼 링이 축을 따라 전파됩니다. QuadriFlow 가 닫힌 경계에 씌운 캡을 지우고 양쪽 링을 최소 비용 브리지로 이으며, 두 링의 정점 수 차이만큼 전이 삼각형이 남아 경고로 보고합니다. `STRIP` 가이드와 밀도 속성, `DIRECTION` 가이드는 반영하지 않으며, 복셀 리메시가 좁은 틈과 내부 면을 닫을 수 있습니다. 경계 엣지 비율이 20%를 넘는 열린 판 형상은 부피가 없어 이 경로를 건너뜁니다. |
...
| 방향 정렬 | 특징선·가이드 4방향장에 이면각 기반 곡률 텐서의 주곡률 방향 앵커를 더해 원통 영역이 축 방향으로 정렬됩니다(비등방성이 낮은 구·평면에서는 가중치가 0 에 가깝습니다). 패치 선택과 정점 정렬. 위에 열거한 제한된 형상 밖의 전역 연속 루프 배치는 미구현 |
| 클릭 링 가이드 | 표면을 클릭하면 클릭 점을 지나는 평면들 가운데 단면 둘레가 짧고 앞뒤로 옮겨도 안정적이며 면 법선이 축과 수직인 방향을 축으로 골라 실제 단면 커브를 만듭니다. 손으로 원을 그릴 때 생기는 크기·기울기 오차가 없고, 절단면 양쪽 둘레 비율을 즉시 표시합니다. 축 오프셋으로 재슬라이스합니다. |
| Guide Curves | 열린 기본 커브는 방향 힌트입니다. 지원되는 독립 관의 닫힌 `LOOP`와 양끝까지 이어지는 `STRIP`은 출력 링·엣지 열로 고정합니다. 단일 두부의 눈·입·코·귀 바깥 `LOOP` 여섯 개를 각각 폐경로로 만듭니다. 둥근 T에는 몸통 링 둘, 팔 링 하나, 팔 방향 `STRIP` 하나가 필요합니다. 격자 경로가 다루지 못하는 형상에서는 닫힌 `LOOP` 가 QuadriFlow 경로의 절단 링이 되어 팔·다리에 링 격자를 만들고, 결과 경고에 접합 링 정점 수·전이 삼각형 수·양쪽으로 이어진 닫힌 링 수를 보고합니다. `STRIP` 은 QuadriFlow 경로에서 보존하지 못하며, 필수 경로를 어느 경로도 보존할 수 없으면 중단합니다. |
...
5. 팔·다리·목 링은 패널의 **클릭으로 링 가이드 추가**로 만드는 것이 가장 확실합니다. 메시 표면을 좌클릭하면 그 자리의 실제 단면을 잘라 축에 수직인 `REMESH_GUIDE_ring` 커브를 만들고, 절단면 양쪽 둘레 비율을 검사해 패널에 표시합니다(우클릭·ESC 로 종료). 추가 중에는 Ctrl+Z 나 Backspace 로 마지막 링만 취소할 수 있고, 끝낸 뒤에는 패널 목록의 X 버튼으로 개별 삭제, 쓰레기통 버튼으로 모두 삭제합니다(일반 오브젝트라 선택 후 X 로 지워도 됩니다). 가이드를 선택하면 **축 오프셋**으로 축을 따라 옮길 수 있고 단면이 다시 잘립니다. 붉은 표시(둘레 비율 0.5 미만)는 평면이 발등·가슴 같은 이웃 부위를 함께 지나는 위치이므로 단면이 일정한 곳으로 옮겨 주세요.
6. 방향 힌트가 필요하면 Curve 이름에 `REMESH_GUIDE_` 접두사를 붙입니다. POLY와 BEZIER를 지원하며 NURBS는 오류로 알립니다. 기본 열린 커브는 `DIRECTION`입니다. 닫힌 커브는 `LOOP`, 이름이 `REMESH_GUIDE_STRIP_`으로 시작하는 열린 커브는 `STRIP`으로 수집합니다. 필수 경로를 만들 수 없는 경우에는 결과를 생성하지 않습니다.
