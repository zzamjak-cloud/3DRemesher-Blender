...
작성일: 2026-09-20. 상태: **큐브, 독립된 열린 삼각 튜브, 단일 구형 두부의 여섯 가이드 폐루프와 사각 축 정렬 T형 몸통·팔의 첫 연결 격자를 검증했다. 실제 얼굴·관절·큰 캐릭터의 전체 기준은 통과하지 않았다.**
...
아래 계약은 목표 구조다. 현재 로컬 구현은 `EngineInput`에 가이드 종류·닫힘 여부와 엔진 모드를 추가했고, `planar.py`·`periodic.py`의 제한된 격자 생성기, `guided_surface.py`의 삼각화된 규칙 격자 복원기, 독립 열린 삼각 관·단일 구형 두부·사각 T형 연결부의 생성기를 연결했다. `irregular_tube.py`는 불규칙 삼각 관을 단면 교차로 처리하고, `rounded_branch.py`는 축 정렬 원형 단면과 네 가이드가 정확히 맞는 단일 닫힌 T형 연결부를 처리한다. 두부 생성기는 앞·양측 세 큐브 투영면에 놓인 눈·입·코·귀의 볼록한 바깥 폐곡선 여섯 개에 한정된다. 돌출형 합성 얼굴에서도 그 경로를 확인했지만 임의의 캐릭터에 새 얼굴 루프를 설계하는 배치기는 없다.
...
현재 로컬 구현은 공평면의 네 경계 패치를 공유 정수 분할 수로 연결하고, 두 열린 경계 사이의 주기 관에 동일 둘레 분할과 길이 방향 연속 열을 만든다. 관에 대응하는 필수 `LOOP`와 `STRIP`은 출력의 링·세로 열로 고정한다. 불규칙하게 삼각화된 독립 관에도 두 평면 열린 경계와 중간의 단일 별모양 단면을 확인할 수 있으면 새 격자를 만든다. 삼각화되기 전에 규칙 쿼드 격자가 있던 닫힌 합성 얼굴은 삼각 대각선을 제거하고 세 개의 독립 `LOOP`와 양쪽 쿼드 띠를 확인한다. 별도의 생성기는 단일 닫힌 구형 두부의 앞·양측 세 큐브 투영면에 볼록한 눈 둘·입·코·귀 둘의 바깥 폐곡선 여섯 개를 새 쿼드 격자에 배치한다. 돌출형 합성 얼굴에서도 이 폐루프를 검증했다. 루프 내부도 쿼드로 채우므로 실제 눈꺼풀·입술 띠나 개구부를 만들지는 않는다. 팬형 전이와 폴, 차트 경계의 압축도 남아 있다. 사각 축 정렬 T형의 단일 닫힌 연결 격자에 더해, 세 원형 가이드 링과 팔 방향 `STRIP`을 명시한 축 정렬 둥근 T형 합성 연결부도 처리한다. 축이 휘거나 단면·가이드가 맞지 않는 일반 겨드랑이 전이는 아직 지원하지 않는다.

단위 테스트와 격리 Blender 5.2에서 큐브 384쿼드(각 면 8×8, 추가 분기 0), 열린 원통 128쿼드(연속 링 9개), 11,094면 세분화 큐브의 직접 격자 처리, 규칙 격자의 합성 얼굴 3,456쿼드 복원, 새 구형 두부 2,314쿼드(목표 2,400)와 독립 삼각 관 256쿼드(목표 256)를 확인했다. 구형 두부의 세 눈·입 루프만 사용한 목표 5,000에서는 4,892쿼드, 최대 종횡비 4.83, 엣지 길이 변동계수 22.5%였다. 앞·양측 여섯 루프를 사용한 독립 삼각 입력 두 종의 목표 5,000에서는 5,012쿼드, 최대 종횡비 4.85, 양방향 표본 표면 오차 최대 0.00696을 확인하고 격리 Blender 5.2 worker와 정면·좌우 와이어 이미지를 검토했다. 사각 축 정렬 T형 몸통·팔의 두 입력에서는 목표 900에 928쿼드, 최대 종횡비 1과 폐루프 세 개를 확인했다. T형 Blender 5.2 worker에서도 928쿼드, 단일 닫힌 컴포넌트, 가이드 세 개와 원본 보존을 확인했다. T형 결과의 Blender 5.2 정면·측면·사선 와이어를 검토해 일정한 격자, 겨드랑이 모서리의 5가 폴 네 개, 명백한 접힘이 없음을 확인했다. 이번 합성 후속 단계에서는 중간 정점 높이가 불규칙한 열린 삼각 관 256쿼드, 돌출형 얼굴의 바깥 폐루프 여섯 개를 가진 5,130쿼드(목표 5,000, 최대 종횡비 4.900), 둥근 T형 연결부의 1,072쿼드와 세 폐루프·팔 방향선을 각각 격리 Blender worker로 확인했다. 기존 임모탈 메시나 복제본은 이 단계에서 시험하지 않았다. 배치 품질 측정과 양방향 표면 표본 오차 검사는 추가됐지만 신규 native 실행 파일, 임의 캐릭터의 눈꺼풀·입술과 관절 전이, 일반 모델 전체의 품질 합격은 아직 달성하지 못했다.
