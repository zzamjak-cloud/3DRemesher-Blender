...
아래 계약은 목표 구조다. 현재 로컬 구현은 `EngineInput`에 가이드 종류·닫힘 여부와 엔진 모드를 추가했고, `planar.py`·`periodic.py`의 제한된 격자 생성기와 `guided_surface.py`의 삼각화된 규칙 격자 복원기를 연결했다. 임의의 캐릭터에 새 얼굴 루프를 설계하는 배치기는 없다.
...
현재 로컬 구현은 공평면의 네 경계 패치를 공유 정수 분할 수로 연결하고, 두 열린 경계 사이의 주기 튜브에 동일 둘레 분할과 길이 방향 연속 열을 만든다. 튜브에 대응하는 필수 `LOOP`와 `STRIP`은 출력의 링·세로 열로 고정한다. 삼각화되기 전에 규칙 쿼드 격자가 있던 닫힌 합성 얼굴은 삼각 대각선을 제거하고 세 개의 독립 `LOOP`와 양쪽 쿼드 띠를 확인한다. 단위 테스트와 격리 Blender 5.2에서 큐브 384쿼드(각 면 8×8, 추가 분기 0), 열린 원통 128쿼드(연속 링 9개), 11,094면 세분화 큐브의 직접 격자 처리, 합성 얼굴의 3,456쿼드 복원을 확인했다. 배치 품질 측정과 양방향 표면 표본 오차 검사는 추가됐지만 신규 native 실행 파일, 일반 곡면의 새 루프 배치, 얼굴·관절 전이 패치와 전체 품질 합격은 아직 달성하지 못했다.
